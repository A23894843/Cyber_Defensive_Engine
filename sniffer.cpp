/*
 * Cyber Defensive Engine - Packet Sniffer Module
 * Description: Low-level C++ network parser using libpcap. Features Post-Quantum 
 * Cryptography (liboqs) for IPC authentication, port scan detection, and Unix 
 * Domain Socket telemetry relay to the Python backend.
 */
 #include <pcap.h>
 #include <string>
 #include <arpa/inet.h>
 ...
 // --- Paths Configuration ---
 // Absolute paths required as this runs under 'sudo' privileges
 ...
 // --- Globals ---
 ofstream logFile;
 int pipe_fd = -1;    // File descriptor for Named Pipe (Stats/Scans)
 int client_fd = -1;  // File descriptor for connected Python client (UDS)
 int server_fd = -1;  // File descriptor for UDS Server listener
 
 // PQC Globals (liboqs structures)
 OQS_SIG *sig_ctx = nullptr;
 uint8_t *public_key = nullptr;
 uint8_t *secret_key = nullptr;
 
 // Detection Thresholds
 const int SCAN_THRESHOLD = 500;  // Ports hit required to trigger scan alert
 const int TIME_WINDOW_MS = 1000; // Timeframe for port hits (1 second)
 
 // Tracks state per-IP for port scanning logic
 struct ConnectionState {
     set<int> accessed_ports; // Unique ports accessed
     std::chrono::high_resolution_clock::time_point first_packet_time; // Rolling window start
 };
 
 // --- Utility Functions ---
 
 // Thread-safe logging function that outputs to the shared engine log
 void writeLog(const string& message) {
     if (logFile.is_open()) {
         time_t now = time(0);
         char* dt = ctime(&now);
         string timestamp(dt);
         timestamp.pop_back(); // Remove newline character from ctime
         logFile << "[" << timestamp << "] " << message << endl;
     }
 }
 
 // Ensures complete delivery of byte buffers over sockets
 bool send_all(int sock, const uint8_t* data, size_t length) {
     ...
     while (total < length) {
         ssize_t sent = send(sock,
                             data + total,
                             length - total,
                             MSG_NOSIGNAL); // MSG_NOSIGNAL prevents SIGPIPE crashes if Python closes
         if (sent <= 0)
             return false;
         total += sent;
     }   return true;
 ...
 // Post-Quantum Cryptography Initialization Routine
 void init_pqc() {
     OQS_init(); 
     ...
     // Allocate memory for cryptographic keys based on algorithm specifications
     ...
     // 1. Save public key for the Python ML module (Verification)
 ...
 void setup_ipc() {
     // 1. Setup Unix Domain Socket (UDS) Server for high-throughput packet relay
     ...
     // Start listening for the Python connection (max queue 5)
     ...
     // Ensure the pipe file exists on the disk
 ...
 // Packages the raw packet, signs it with ML-DSA-44, and sends it to Python
 void relay_to_python_pqc(const uint8_t* packet_data, uint32_t length) {
     if (client_fd == -1) {
         // Accept new UDS connection if not connected
 ...
 class PacketSniffer {
     ...
     unordered_map<string, ConnectionState> port_scan_monitor; // Tracks port accesses for scan detection
     time_t last_time;
     pcap_t* handle;
     string device;
 ...
 public:
     ...
     // Initialize libpcap interface
     bool init() {
         char errbuf[PCAP_ERRBUF_SIZE];
         // Open live interface with 65535 byte snaplen, promiscuous mode (1), and 10ms timeout
         handle = pcap_open_live(device.c_str(), 65535, 1, 10, errbuf);
         return (handle != NULL);
     }
 
     // Static callback triggered by libpcap for every captured packet
     static void callback(u_char* user, const pcap_pkthdr* header, const u_char* packet) {
         ...
         // 1. Relay payload to Python ML engine over UDS with PQC Signature
         relay_to_python_pqc(packet, header->caplen);
 
         // 2. Traffic Analysis & Port Scan Detection (IPv4 Only)
         // Check if Ethernet packet and Type is IPv4 (0x0800)
         if (header->caplen >= 34 && packet[12] == 0x08 && packet[13] == 0x00) {
             ...
             // Port Scan Logic: Check Protocol (6 = TCP, 17 = UDP)
             int protocol = packet[23];
             if (protocol == 6 || protocol == 17) {
                 // Calculate IP Header length to find Transport Layer
                 ...
                 // Trigger Detection if threshold is breached within 1 second
                 if (state.accessed_ports.size() >= SCAN_THRESHOLD) {
                     ...
                     // Native C++ IPTables drop for immediate mitigation
         ...
         // Periodic Stats delivery (Every 1 second)
 ...
 // Graceful exit handler to clean up system artifacts
 ...
 int main() {
     ...
     // Mutex Lock to prevent multiple sniffer instances
     ...
     init_pqc(); // Init Quantum Cryptography
     setup_ipc(); // Init UDS & Pipes
     ...
     string dev_name = alldevs->name; // Defaulting to first interface (usually eth0 or wlan0)
     ...
     // Block here and process packets indefinitely
     sniffer.start();
 
     // Final Cleanup (Will only run if pcap_loop exits manually)
     handle_exit(0);
     return 0;
 }
